...
int GLwrap::Shader::LoadShader(unsigned int type, std::string filepath) {
  switch (type) {
...
unsigned int GLwrap::Shader::compileShader(unsigned int type) {
  const char *src;
...
void GLwrap::Shader::CreateShader() {
...
void GLwrap::Shader::Bind() { GLDebug(glUseProgram(ShaderProgramID)); }


GLwrap::Shader::uniform::uniform(Shader &shad, std::string uniform_name) {
  shader = &shad;
  name = uniform_name;
}